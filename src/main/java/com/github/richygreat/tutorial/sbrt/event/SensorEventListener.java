...
public class SensorEventListener {
    ...
    public void consumeSensorEvent(@Payload Sensor sensor, @Header("timestamp") Long timestamp) {
        ...
        throw new MessageConversionException("Invalid Message");
    }

    @StreamListener(DltSink.DLT_INPUT)
    public void consumeDltSensorEvent(@Payload Sensor sensor, @Header("timestamp") Long timestamp) {
        log.info("consumeDltSensorEvent: sensor: {} timestamp: {}", sensor, timestamp);
    }
}
